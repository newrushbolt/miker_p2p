...
CREATE TABLE peers_updates (
  ...
  CONSTRAINT peer_conn_id FOREIGN KEY (conn_id) REFERENCES peers (conn_id)
);  
...
CREATE TABLE peers_good (
  ...
  CONSTRAINT seed_conn_id FOREIGN KEY (conn_id) REFERENCES peers (conn_id),
  CONSTRAINT peer_conn_id FOREIGN KEY (peer_conn_id) REFERENCES peers (conn_id),
  CONSTRAINT seed_peer_ts UNIQUE (conn_id,peer_conn_id,ts)
);
...
CREATE TABLE ip_good (
  ...
  bytes int NOT NULL,
  ltime int DEFAULT NULL,
  CONSTRAINT seed_peer_ts UNIQUE (ip,peer_ip,ts)
);
