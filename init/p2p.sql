...
CREATE USER p2p with encrypted password 'p2p' login;
...
-- список сетей с ip информацией, обновляется отдельным приложением
...
CREATE TABLE peers_updates (
  ...
  CONSTRAINT peers_updates_peer_conn_id FOREIGN KEY (conn_id,channel_id) REFERENCES peers (conn_id,channel_id)
);
